...
class Controller():
    ...
    def policyIteration(self):
        ...
        for i in range(self.args.policyUpdates):
            ...
            for e in range(self.args.policyEpisodes):
                ...
                scores = np.append(scores, example)
    ...
    def executeEpisode(self, init_egs=False):
        ...
        for t in range(self.args.goal_steps):
            ...
            example[t, 0] = prev_observation
            example[t, 1:2] = [pi, score]

            prev_observation = self.mcts.updata2Dtraj(observation)
            score += reward  # +1 for every frame we haven't fallen

            if done:
                break

        example[:, 2] = score - example[:, 2]  # Convert scores to E[return]
